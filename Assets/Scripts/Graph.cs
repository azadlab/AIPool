...
public class Graph 
{
	...
	public virtual SPath GetShortestPath ( Node start, Node end )
	{
		...
		while ( unvisited.Count != 0 )
		{
			...
			for ( int i = 0; i < current.connections.Count; i++ )
			{
				...
				float length = current.distance(neighbor);
	...
	public List<Node> GetAllNodes(string name)
	{
	...
	public bool isNode(Node n)
	{
		for(int i=0;i<m_Nodes.Count;i++)
		{
			if(m_Nodes[i].distance(n) < Node.MIN_NODE_DIST)
			{
				m_Nodes[i] = n;
				return true;
			}
	...
	public int GetNodeIdx(Node n)
	{
		int idx = 0;
		for(idx=0;idx<m_Nodes.Count;idx++)
		{
			if(m_Nodes[idx].distance(n) < Node.MIN_NODE_DIST)
			{
				return idx;
			}
	...
	{
		int idx1 = GetNodeIdx(n1);
		int idx2 = GetNodeIdx(n2);

		m_Nodes[idx1].AddConnection(m_Nodes[idx2]);
		m_Nodes[idx2].AddConnection(m_Nodes[idx1]);
